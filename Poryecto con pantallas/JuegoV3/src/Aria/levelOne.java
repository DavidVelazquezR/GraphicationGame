...
import Hygel.DrawMagician;
import static Hygel.MagicianV2.tec;
...
public class levelOne extends JFrame implements GLEventListener, KeyListener, MouseListener, MouseMotionListener {
    ...
    public void main()  {
        ...
        canvas.addGLEventListener(new levelOne()); //con la clase en la que este el personaje
        frame.add(canvas);
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        if (typeCharacter == 1) {
            ...
            //Dibuja la figura 3d dependiendo de la tecla que se presione
            //Mueve la escena en la psoicion de la matriz
            gl.glTranslatef(coordXPersonaje, mage.Z_POSITION, 0.0f);
            gl.glRotatef(rotFigure, 0.0f, 1.0f, 0.0f);
            mage.draw_mage(gl, controlActions);
        }
                    ...
                    if (!mage.up) {
                        Thread hilo = new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                BigDecimal bd = new BigDecimal(mage.Z_POSITION);
                                bd = bd.setScale(1, RoundingMode.HALF_UP);
                                System.out.println(bd);

                                mage.Z_POSITION = bd.floatValue();
                                System.out.println("saltoo");
                            }
                        });
